...
import (
	...
	"github.com/WildfireData/go-sdk/v2/src/wildfiredata"
...
func main() {
	// enable console log
	wildfiredata.SetLogLevel(wildfiredata.TDLogLevelError)
...
func syncExample(te *wildfiredata.TDAnalytics, accountId, distinctId string, properties map[string]interface{}) {
	//sync
	for j := 0; j < 1; j++ {
...
func asyncExample(te *wildfiredata.TDAnalytics, accountId, distinctId string, properties map[string]interface{}) {
	wg := sync.WaitGroup{}
	for i := 0; i < 1; i++ {
		wg.Add(1)
		distinctId = randomString()
...
func generateLogConsumer() (wildfiredata.TDConsumer, error) {
	// logConsumer config
	config := wildfiredata.TDLogConsumerConfig{
		FileNamePrefix: "test_prefix",
		Directory:      "./log",
		RotateMode:     wildfiredata.ROTATE_HOURLY,
		FileSize:       200,
	}
	return wildfiredata.NewLogConsumerWithConfig(config)
}

func generateBatchConsumer() (wildfiredata.TDConsumer, error) {
	config := wildfiredata.TDBatchConfig{
		ServerUrl: "https://receiver-ta-uat.wildfiredata.cn",
		AppId:     "your appId",
		AutoFlush: true,
		BatchSize: 100,
		Interval:  20,
	}
	return wildfiredata.NewBatchConsumerWithConfig(config)
}

func generateDebugConsumer() (wildfiredata.TDConsumer, error) {
	return wildfiredata.NewDebugConsumerWithDeviceId("https://receiver-ta-uat.wildfiredata.cn", "appid", false, "deviceId")
}
